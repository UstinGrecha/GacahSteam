...
import { serverOpenPack } from "@/lib/server/onlineGame";
...
export async function POST(req: Request) {
  ...
  if (!isPackKind(o.packKind) || !isSeries(o.series)) {
    return NextResponse.json({ error: "invalid_body" }, { status: 400 });
  }
  if (!isSeriesPacksAvailable(o.series)) {
    return NextResponse.json({ error: "series_unavailable" }, { status: 400 });
  }

  const result = await serverOpenPack(userId, o.packKind, o.series);
  if (!result.ok) {
